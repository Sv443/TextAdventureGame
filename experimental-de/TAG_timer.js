	...
	if(!document.body.innerHTML.includes("id='status_dead'")){
		switch(t_event){
			...
			case "found_shipwreck":
				sendmsg("Du liefst auf der Insel herum und hast ein Schiffswrack gefunden. Vielleicht kannst du es erkunden?");
				incscore(150);
				getitem("structure_shipwreck", "Schiffswrack - Ein toller Unterschlupf und es kann erkundet werden! (Struktur-Effekte: gibt Schutz vor Regen und kann erkundet werden)", "https://raw.githubusercontent.com/Sv443/TextAdventureGame/master/shipwreck_16x16.png", "appendstructure")
				break;
		}
	}
}
...
function startTimer(duration, display) { //timer code from https://stackoverflow.com/a/20618517
    ...
    setInterval(function () {
		if(!document.body.innerHTML.includes("You died")){
			...
			else if(!document.body.innerHTML.includes("ein Schiffswrack gefunden") && minutes == time_found_boat && seconds == 00){	timedevent("found_shipwreck");}
			
			else if(unlock_all_timed && !document.body.innerHTML.includes("ein Schiffswrack gefunden")){timedevent("found_shipwreck");}
